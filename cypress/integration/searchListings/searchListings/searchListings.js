...
Then('Clicking On Search Button Should Display The Listing Results', () => {
    cy.intercept(
        'GET',
        `https://pkycbn6yz2.execute-api.us-east-1.amazonaws.com/test/api/referencedata/performance?q=Global&take=50&skip=0`
    ).as('searchListings');
    cy.get(powerAppLocators.searchButton).should('exist');
    cy.get(powerAppLocators.searchButton).click();
    cy.wait('@searchListings').then((intercept) => {
...
Then('I Should See The Performance Error Message {string} For {string}',(errorMessage, searchString) => {
    cy.intercept(
        'GET',
        `https://pkycbn6yz2.execute-api.us-east-1.amazonaws.com/test/api/referencedata/performance?q=${searchString}&take=50&skip=0`
    ).as('searchListings');
    cy.get(powerAppLocators.searchButton).should('exist');
    cy.get(powerAppLocators.searchButton).click();
    cy.wait('@searchListings').then((intercept) => {
